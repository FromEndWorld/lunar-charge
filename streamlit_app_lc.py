...
st.caption("基于16种暴击组合精确计算期望伤害 | 最高伤害×1，次高×0.5，第三第四×0.083 | 作者：GPT-4")

# 角色数据输入表单
st.header("角色参数设置")
cols = st.columns(4)
characters = []
weights = [1.0, 0.5, 1/12, 1/12]  # 伤害权重

for i in range(4):
    with cols[i]:
        st.subheader(f"角色 {i+1}")
        name = st.text_input(f"角色名", key=f"name_{i}", placeholder="例如：雷电将军")
        ...
        with st.expander("计算原理说明"):
            ...
            **精确计算流程**:
            ...
            2. 对每种组合：
               ...
               - 按伤害值从高到低排序
               - 应用权重系数：最高伤害×1.0，次高×0.5，第三第四×0.083
               - 计算该组合的加权总伤害
               - 期望贡献 = 加权总伤害 × 发生概率
            ...
            *注：公式参数可根据游戏实际机制调整*
            ...
            # 显示所有组合的数学表达式
            st.subheader("所有组合表达式")
            for i, scenario in enumerate(scenario_results):
                expr = f"组合 {i+1}: {scenario['组合']} → "
                expr += f"P = {scenario['概率']:.6f}, "
                expr += f"伤害 = {int(scenario['加权伤害'])}, "
                expr += f"贡献 = {scenario['期望贡献']:.2f}"
                st.code(expr)
else:
    st.info("请输入角色参数后点击「精确计算伤害期望」按钮")

# 页脚
st.divider()
st.caption("原神月感电伤害计算器 v3.0 | 基于精确组合计算 | 数据仅供参考，实际游戏效果以官方为准")